...
namespace SistemaGestion
{
    public partial class frmProducto : Form
    {
        ...
        {
            if (decimal.TryParse(txtCosto.Text, out decimal value))
            {
                txtCosto.Text = value.ToString("0.00");
            }
        }

        private void txtPrecioVenta_TextChanged(object sender, EventArgs e)
        {
            if (decimal.TryParse(txtPrecioVenta.Text, out decimal value))
            {
                txtPrecioVenta.Text = value.ToString("0.00");
            }
        }
